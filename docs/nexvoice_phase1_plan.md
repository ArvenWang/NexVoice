...
这次要做的不是完整复刻 Typeless，而是做一个更聚焦、更适合个人开发落地的 Mac 工具。产品名定为 **NexVoice**。
...
2026-06-18 更新：这一节是早期建议，已经被最新产品决策覆盖。当前用户明确速度第一，NexVoice 默认 ASR 已切到腾讯云实时语音识别大模型；SenseVoice Small via sherpa-onnx 与 WhisperKit large-v3 保留为本地兜底和质量对照。

早期结论：**第一版主链路建议云端，端侧先预留接口。**
...
1. 新工具已命名为 NexVoice，后续需要确认正式 Bundle ID、图标和展示名。
