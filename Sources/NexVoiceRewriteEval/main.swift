...
@main
struct RewriteEvaluationTool {
    static func main() async {
        let rawArguments = Array(CommandLine.arguments.dropFirst())
        let arguments = Set(rawArguments)
        let dryRun = arguments.contains("--dry-run")
        let includePrompt = arguments.contains("--include-prompt")
        let outputPath = value(after: "--output", in: rawArguments) ?? defaultReportPath()

        let scenarios = makeScenarios()
        let results: [EvaluationResult]
        if dryRun {
            results = scenarios.map { scenario in
                EvaluationResult(
                    scenario: scenario,
                    latencyMs: nil,
                    timeoutSeconds: VoiceRewriteTimeoutPolicy.timeoutSeconds(
        ...
        let reportText = report(results: results, dryRun: dryRun, includePrompt: includePrompt)
        print(reportText)
        writeReport(reportText, to: outputPath)
    }
    ...
    private static func writeReport(_ report: String, to path: String) {
        ...
        do {
            ...
            fputs("Failed to write report: \(error.localizedDescription)\n", stderr)
        }
    }
    ...
    private static func defaultReportPath() -> String {
        ...
        let fileName = "deepseek-rewrite-eval-\(formatter.string(from: Date())).md"
    ...
    private static func makeScenarios() -> [EvaluationScenario] {
    ...
    private static func runScenarios(_ scenarios: [EvaluationScenario]) async -> [EvaluationResult] {
    ...
    private static func runScenario(
        ...
        do {
            ...
            let checks = makeChecks(for: scenario, output: output)
    ...
    private static func guardedOutput(_ output: String, for scenario: EvaluationScenario) -> String {
    ...
    private static func selectedTextCharacters(for scenario: EvaluationScenario) -> Int? {
        guard case .selectedTextCommand(let selectedText, _) = scenario.operation else {
            return nil
        }
        return selectedText.count
    }

    private static func finalRewriteSourceText(for scenario: EvaluationScenario) -> String? {
    ...
    private static func sourceTextCharacters(for scenario: EvaluationScenario) -> Int? {
    ...
    private static func makeChecks(for scenario: EvaluationScenario, output: String) -> [(String, Bool)] {
    ...
    private static func report(
        ...
        var lines: [String] = []
        lines.append("# NexVoice DeepSeek 上下文评测")
        lines.append("")
        lines.append("- 模式：\(dryRun ? "dry-run，仅检查 prompt" : "真实请求 DeepSeek")")
        lines.append("- 场景数：\(results.count)")
        lines.append("")
        ...
        for result in results {
            ...
            if includePrompt || dryRun {
                lines.append("Prompt 片段：")
                lines.append("```text")
                lines.append(String(result.scenario.prompt.prefix(2_400)))
                lines.append("```")
                lines.append("")
            }
            ...
            if let error = result.error {
                ...
                lines.append("结果：dry-run 未发起模型请求。")
            }
            lines.append("")
        }
