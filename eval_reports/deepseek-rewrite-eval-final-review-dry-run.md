...
输出模式：
使用创意发散模式：这是最高改写幅度，只在用户手动选择时使用。允许更大胆地重组句子、增强表达张力、加入更鲜明的个性，让文字更抓人、更不普通；但仍然不能新增事实、不能改变核心立场，不能使用羞辱性、冒犯性、死亡、暴力或粗俗比喻，也不要写成尴尬段子或过度口号。即使风格更强，也必须输出普通纯文本，禁止用 **、#、反引号、引用块等 Markdown 符号来制造强调。
