...
- 看屏回复已接入：按住默认右 Alt 会同时截取当前前台应用最大可见窗口并监听语音指令；用户可在按住期间说“用更强硬的语气回”“回复第二段”“简短一点”等要求，松开右 Alt 后结束识别。DeepSeek 会结合可见文字、粗略的“我 / 对方 / 未知”结构和语音指令生成一条新回复；如果没有识别到语音指令，则按可见上下文正常回复。该模式只读取屏幕上可见内容，不会滚动或读取屏幕外历史，也不会自动发送。
...
- 看屏回复使用极简监听状态条，不复用普通语音输入的大文本框、实时转写文字或波形条。
...
- 默认快捷键为右 Alt：短按触发语音输入，长按约 0.55 秒触发看屏回复；菜单中可录制自定义快捷键。
- 全局快捷键实现按 NexHub 现有 `NSEvent` monitor 模式接入；菜单提供辅助功能权限和屏幕录制权限入口，分别用于接收全局键盘事件 / 写入当前 App，以及读取当前窗口可见文字。
- 本地 `.app` 构建脚本：`./scripts/build_app.sh debug`。
- 版本号规则：每次进入 Git 同步的真实迭代都必须递增 `0.0.1`，同时递增 build 号。运行 `./scripts/install_git_hooks.sh` 后，本仓库的 pre-commit hook 会在提交包含代码、文档或配置改动时自动完成版本递增，并同步更新三个 App / 工具的 `Info.plist`。
