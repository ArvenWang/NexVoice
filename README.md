...
- 看屏回复已接入：长按默认右 Alt 会截取当前前台应用最大可见窗口，用 Apple Vision 做本地 OCR，然后进入语音指令录制；用户可说“用更强硬的语气回”“回复第二段”“简短一点”等要求，再按一次右 Alt 结束。DeepSeek 会结合可见文字、粗略的“我 / 对方 / 未知”结构和语音指令生成一条新回复，写入当前输入框但不自动发送。该模式只读取屏幕上可见内容，不会滚动或读取屏幕外历史。
