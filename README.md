...
- DeepSeek 最终整理 Provider，用于把腾讯云 final 文本整理成自然、清晰、有逻辑的最终稿；菜单可选择输出中文或英文，并通过二级菜单选择单一 `输出模式`。默认模式为 `忠实整理`，手动项为 `清晰优化`、`自然表达`、`专业严谨`、`增强表达`、`疯狂模式`；超时会按普通输入、长文本、选中文本指令和疯狂模式动态调整，失败或超时时写入原始 ASR 文本。
