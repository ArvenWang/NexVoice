...
## 本轮追加（2026-07-02：波形未解决问题记录与远端同步）

- 当前状态：
  - 已继续尝试让静音状态保持低亮度噪波运动，并提高静音相位推进速度；本地安装包与 `/Applications/NexVoice.app` 的可执行文件哈希一致，说明安装已生效。
  - 代码层采样显示静音 active 状态下格子强度会随 phase 变化，但用户实测视觉上仍像完全静止，说明问题可能在“低亮度变化肉眼不可见”或运行状态没有让 `VoiceWaveformView` 保持 active。
- 用户明确反馈的未解决问题：
  - 静音状态下仍然没有可见运动，也几乎没有亮度变化；静音时应该仍有正常的噪波流动，未亮起的格子也要随机变化，而不是定住。
  - 说话时亮的地方有一点过于碎，噪波碎片值需要往回收一些，让高亮区域更聚合、更稳定。
- 下一步建议：
  - 优先补一条波形运行诊断日志，记录 `isActive`、`phase`、`amplitude`、reduce motion 状态和低亮度格子的实际绘制强度，先确认是视图没在动，还是动了但视觉映射太弱。
  - 如果诊断确认 phase 正常推进，应调整低亮度区的颜色/亮度映射，而不是继续只改噪波数学值；同时降低说话态的碎片噪波权重。
...
- 本轮结论：
  - 已把波形从偏青的蓝色系改为以 `#4E18FF` 为基准的蓝紫色系，亮部过渡到淡蓝紫，整体更偏科技蓝紫。
  - 静音状态不再使用偶发高噪波闪点；保留低不透明度的局部纺锥底，并只叠加小幅噪波运动，避免单个像素忽然接近消失。
  - 静音时不会把全部格子打开：采样显示 220 个格子中约 119 个达到可见阈值，仍保留自然渐隐和局部噪波。
- 已执行：
  - `VoiceWaveformDisplayPolicy`：移除静音 `idleTrace` 偶发闪点，把静音 ambient noise 从大幅波动改成稳定底值 + 小幅扰动。
  - `VoiceCaptionPanelController`：颜色映射改成蓝紫基准；静音 phase 基础速度继续降低，说话时仍随音量加速。
  - `VoiceWaveformDisplayPolicyTests`：新增 `silentActiveWaveformKeepsIndividualPixelsStable`，约束静音可见像素在 60 帧内不能明显掉到快消失，同时确认不是所有像素都可见。
  - 运行 `./scripts/bump_version.sh`，版本从 `0.1.73 (74)` 升到 `0.1.74 (75)`。
  - 已构建并安装带 API 配置的 `/Applications/NexVoice.app`，旧版备份为 `dist/install-backups/NexVoice-20260702-023407-pre-waveform-violet-stable-idle.app`。
  - 已重启安装版 App，当前进程 PID `11903`。
  - 已生成带 API 配置的分享 DMG：`dist/NexVoice-0.1.74-build75-violet-stable-idle-embedded-keys-20260702.dmg`。
  - DMG SHA256：`e9e3d0f3d4a80961c591089157953a4900adab3807ec76b2d659c72fd03c1579`。
- 已验证：
  - `swift test --disable-sandbox --filter VoiceWaveformDisplayPolicyTests` 通过（17 tests）。
  - `swift test --disable-sandbox --quiet` 通过（162 tests）。
  - `./scripts/build_app.sh release --embed-local-keys` 通过。
  - `codesign --verify --deep --strict --verbose=2 dist/NexVoice.app` 通过。
  - `codesign --verify --deep --strict --verbose=2 /Applications/NexVoice.app` 通过。
  - `plutil -lint dist/NexVoice.app/Contents/Info.plist /Applications/NexVoice.app/Contents/Info.plist` 通过。
  - `dist/NexVoice.app` 和 `/Applications/NexVoice.app` 内的 `DeepSeek.json`、`TencentCloudASR.json` 嵌入配置存在且非空。
  - `hdiutil verify dist/NexVoice-0.1.74-build75-violet-stable-idle-embedded-keys-20260702.dmg` 通过。
  - 已挂载 DMG 验证根目录包含 `NexVoice.app` 和 `Applications` 快捷入口，且 DMG 内 App 签名通过、嵌入配置存在且非空。
...
- 本轮结论：
  - 已按最新反馈压低默认/轻声状态亮度，把主要亮度对比留给说话时。
  - 说话时中心像素会更快进入高亮，白紫色高光更强，并通过噪波控制的纺锥能量场向两侧扩散。
  - 默认状态仍保留细碎流动噪波，但不会再提前接近满亮；两侧亮点继续保持稀疏，避免整条均匀发光。
- 已执行：
  - `VoiceWaveformDisplayPolicy`：改用更晚触发的音量响应曲线，新增弱环境噪波层，增强说话时中心能量和向外扩散的 spindle 区域，并提高侧边碎点门槛。
  - `VoiceCaptionPanelController`：调整像素颜色映射，低强度像素更暗，高强度像素更接近明亮白紫色。
  - `VoiceWaveformDisplayPolicyTests`：新增/更新测试覆盖轻声低亮度、说话中心显著变亮、亮区范围扩大、侧边碎点稀疏和默认噪波仍流动。
...
- 本轮结论：
  - 已撤回上一轮“全宽暗底”方向，改为无暗点底色的透明矩阵发光。
  - 声音现在主要控制整体明暗和噪波门控强度，不再用整齐的线性渐变直接决定宽度。
  - 正常说话时，中间区域明显更亮；上下五行的能量不是均匀分布，而是形成中间厚、上下和左右弱的纺锥形基础，再叠加碎噪波。
  - 两侧仍会出现少量细碎亮点，但不是均匀渐暗，也不是整条暗底常亮。
- 已执行：
  - `VoiceWaveformDisplayPolicy`：改为“纺锥形能量场 + 噪波门控”模型；提高音量响应阈值和强度，让正常说话时中心亮度更明显。
  - `VoiceCaptionPanelController`：移除暗底透明度映射，低强度像素基本不可见；动画速度继续随音量上升。
  - `VoiceWaveformDisplayPolicyTests`：新增/更新测试覆盖无暗点底色、纺锥形五行分布、中心亮度强于两侧、两侧稀疏亮点和非方向性噪波。
  - 运行 `./scripts/bump_version.sh`，版本从 `0.1.69 (70)` 升到 `0.1.70 (71)`。
  - 已构建并安装带 API 配置的 `/Applications/NexVoice.app`，旧版备份为 `dist/install-backups/NexVoice-20260702-020512-pre-waveform-spindle-noise.app`。
  - 已重启安装版 App，当前进程 PID `63983`。
- 已验证：
  - `swift test --disable-sandbox --filter VoiceWaveformDisplayPolicyTests` 通过（14 tests）。
  - `swift test --disable-sandbox --quiet` 通过（159 tests）。
  - `./scripts/build_app.sh release --embed-local-keys` 通过。
